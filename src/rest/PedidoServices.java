...
//     */
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response cancelarPedido(InfoPedido infoPedido) {
		RotondAndesTM tm = new RotondAndesTM(getPath());
		InfoPedido retorno = null;
		try {
			retorno = tm.resgistrarProductosMesaGlobal(infoPedido);
		} catch (Exception e) { 
			return Response.status(500).entity(doErrorMessage(e)).build();
		}
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response disableRestauranteGlobal(@PathParam("nombre") String nombre) {
		RotondAndesTM tm = new RotondAndesTM(getPath());
		try {
			;
		} catch (Exception e) {	
