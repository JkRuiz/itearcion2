...
//	}
	...
	@Produces(MediaType.APPLICATION_JSON)
	public Response addMenu(Menu menu, @PathParam( "idPlato" ) int idPlato){
		RotondAndesTM tm = new RotondAndesTM(getPath());
		try {
			tm.addMenuPlato(menu, idPlato);
		} catch (Exception e) {
			return Response.status(500).entity(doErrorMessage(e)).build();
		}
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response addPlatoMenu(Menu menu, @PathParam( "idPlato" ) int idPlato){
		RotondAndesTM tm = new RotondAndesTM(getPath());
		try {
			tm.addPlatoMenu(menu, idPlato);
		} catch (Exception e) {
			return Response.status(500).entity(doErrorMessage(e)).build();
		}
		return Response.status(200).entity(menu).build();
